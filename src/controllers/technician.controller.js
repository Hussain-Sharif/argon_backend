    ...
    if(dbUserResult===undefined){
       return  res.status(400).json(new ApiError(400,"Business not found"))
        ...
        if(isPasswordMatch){
            ...
            res.status(200).cookie("jwtToken",jwtToken).json(new ApiResponse(200,"Business logged in Successfully",{jwtToken,name:dbUserResult["name"],type:"technician",email:dbUserResult["email"],image:dbUserResult["image"]})) // directly storing in cookies of browser or we can get token from response directly
        }else{
